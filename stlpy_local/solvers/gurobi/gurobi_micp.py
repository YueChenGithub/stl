...
class GurobiMICPSolver(STLSolver):
    ...
    def Solve(self):
        ...
        return (x, u, rho, self.model.Runtime, self.zt.X, self.ct1.X, self.ct0.X, self.chara.X)
    ...
    def AddSubformulaConstraints(self, formula, z, t):
        ...
        if isinstance(formula, LinearPredicate):
            # temporal robustness measure
            # rho = formula.a.T @ self.y[:, t:self.T] - formula.b
            # zt = self.model.addMVar(self.T - t, vtype=GRB.BINARY)
            # for i in range(self.T - t):
            #     self.model.addConstr(rho[0, i] >= 0 - self.M * (1 - zt[i]))
            #     self.model.addConstr(rho[0, i] <= 0 + self.M * zt[i])
            #
            # zt_int = self.model.addMVar(self.T - t, vtype=GRB.INTEGER, lb=-100)
            # self.model.addConstr(zt_int == zt)
            # chara = 2 * zt_int - 1
            # ct1 = self.model.addMVar(self.T - t + 1, vtype=GRB.INTEGER, lb=-100)
            ...
            # ct0_ = ct0[:-1] + (1 - zt)
            #
            # theta = ct1_ + ct0_ - chara
            # self.model.addConstr(theta[0] + (1 - z) * self.M >= self.rho)
            ...
            theta = ct1[:-1] + ct0[:-1]  # temporal robustness
            ...
            if mode == 1:
                robustness = theta[t]
            if mode == 2:
                robustness = (formula.a.T @ self.y[:, t] - formula.b) + (theta[t])
